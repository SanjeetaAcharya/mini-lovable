...
export function BillingView({ balance, packs, history, onBuy, buyingPackId, loading }: Props) {
  ...
  return (
    <div className="space-y-8">
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className={`mt-5 w-full rounded-full py-2 text-sm disabled:cursor-not-allowed disabled:border-ink-700 disabled:text-fg-dim ${
                  best
                    ? "bg-accent font-medium text-ink-950 hover:bg-accent/90"
                    : "border border-ink-500 text-fg hover:bg-ink-800"
                }`}
              >
      ...
      <div>
        <div className="mb-3 text-[10px] font-semibold tracking-widest text-fg-muted uppercase">
          Purchases
        </div>
        <div className="overflow-x-auto rounded-lg border border-ink-800 bg-ink-900">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {purchases.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-sm text-fg-muted">
