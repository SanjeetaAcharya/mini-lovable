...
- Public deployment. Frontend on Vercel (`https://mini-lovable-web.vercel.app`), backend on Render (`https://mini-lovable-api.onrender.com`), both auto-deploying from `main`. **Why Render and not Vercel for the backend:** the obvious worry — Vercel's function timeout vs. 30s+ generations — turned out to be a non-issue (Hobby now defaults to 300s with Fluid Compute, and Vercel supports zero-config Express), but two real reasons stand: the Phase 8 Docker sandbox needs a container runtime that serverless functions don't have, and Prisma-in-serverless reintroduces exactly the connection-pool-exhaustion failure mode the previous phase was spent hardening against. Render runs the existing `app.listen()` process unmodified. **Why Render and not Railway:** Railway's free tier is now a one-time $5/30-day trial credit, then $5/mo minimum — not a sustained free tier, which breaks the project's zero-budget constraint. Render's free web service is genuinely perpetual; the honest tradeoff is that it spins down after 15 minutes idle, so the first request after a gap takes ~30-60s. Code changes required: the frontend's API base URL became configurable (`VITE_API_URL`, baked in at build time) instead of a hardcoded relative `/api` that only worked behind the Vite dev proxy — including the invoice `<a href>`, which would otherwise have pointed at a path that doesn't exist on the frontend's own origin; and backend CORS moved from wide-open `cors()` to `origin: CLIENT_URL`. Stripe: a real webhook endpoint was registered against the deployed API via the Stripe API, and its production signing secret (different from the one `stripe listen` prints locally) set on Render.

  Three real bugs surfaced only once deployed, none of which could have appeared locally. (1) `CLIENT_URL` was set with a trailing slash, which produced `vercel.app//purchase/success` in Stripe's redirect and — more subtly — broke CORS outright, since `Access-Control-Allow-Origin` must *exactly* equal the browser's `Origin` header, which never carries a trailing slash; fixed by stripping it in code rather than relying on the env var being typed correctly. (2) `STRIPE_WEBHOOK_SECRET` was missing on Render; diagnosed precisely by sending a correctly-signed probe request and reading the failure mode — "key must be a string, received undefined" (var absent) is a different error than "no signatures found matching" (var present but wrong). (3) After paying, customers landed on a **Vercel 404** instead of the app: this is a client-side-routed SPA whose only real file is `index.html`, and Vite's dev server silently does SPA fallback for unknown paths while Vercel does not — so `/purchase/success` worked in dev and 404'd in production. Fixed with a `rewrites` rule in `apps/web/vercel.json`. The ledger credit had still been happening correctly throughout (the webhook is server-side and independent of the frontend), but the customer saw a broken page.
